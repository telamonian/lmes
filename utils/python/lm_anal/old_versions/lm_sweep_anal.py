...
pCMap = {}
rPCMap = {}
walk = next(os.walk(os.path.join(os.path.dirname(os.path.realpath(__file__)),'lm_anal','src','plot','colormaps')))
...
#         self.p = []
    ...
    def FFluxProb(self, axes=None, fig=None, logScaledY=False, s=100):
        if axes==None:
            axes = plt.axes()
        if fig==None:
            fig = plt.figure(1)
...
#         print(np.linspace(-22.5,22.5,12),)
        ...
        # normalize by just the middle values
#         normP = normP/np.sum(normP[normP.shape[0]/2 - 2:normP.shape[0]/2 + 3])
        # use the middle of the bins as the x values
        xVals = (np.linspace(-25,25,13)[1:] + np.linspace(-25,25,13)[:-1])/2
        # divide through by the (uniform) bin width to convert this pmf to a pdf
        normP/=(xVals[1] - xVals[0])
#         print(np.array([xVals, normP]).T)
        normCheck = np.sum(normP*(float(50)/12))
        print('fflux normCheck is: %.10f' % normCheck)
        im = axes.scatter(xVals, normP, c='r', s=s)
...
#         axes.set_xlabel('$\Delta$ (copy num(b) - copy num(a))', fontsize=14)
    ...
    def Hist(self, axes=None, fig=None, fontSize=20, fontSizeTicks=14, hideLastTick=False, logScaledY=False, normed=False, normedRange=(-22.5,22.5), saveFig=True):
        if axes==None:
            axes = plt.axes()
        if fig==None:
            fig = plt.figure(1)
...
#                 rightIndex = np.searchsorted(self.histogram1DNormed[:,0], normedRange[1])
                ...
                self.histogram1D = self.histogram1D[74:125,:]     #[74:125,:]
#             print(self.histogram1D)
                self.histogram1DNormed = copy.deepcopy(self.histogram1D)
            # normalize to match binning of ffluxProb
#             self.histogram1D[0,1]/=2
#             self.histogram1D[0,-1]/=2
#             self.histogram1DNormed = np.zeros((12,2))
#             self.histogram1DNormed[:,0] = (np.linspace(-25,25,13)[1:] + np.linspace(-25,25,13)[:-1])/2
#             self.histogram1DNormed[:,1] = np.histogram(self.histogram1D[:,0], weights=self.histogram1D[:,1], bins=np.linspace(-25,25,13), density=True)[0]
#             self.histogram1DNormed = np.copy(self.histogram1D)
            # normalize only over the range -25 to 25. Assume that this is a pdf, so bins are centered around each integer value and extend to +/- half values (ie the bin corresponding to 14 has edges at 13.5 and 14.5)
            self.histogram1DNormed[:,1] =  self.histogram1D[:,1]/(np.sum(self.histogram1D[1:-1,1]) + (self.histogram1D[0,1] + self.histogram1D[-1,1])/2)
            # normalize by just the middle values
#             self.histogram1DNormed[:,1] =  self.histogram1D[:,1]/np.sum(self.histogram1D[self.histogram1D.shape[0]/2 - 4:self.histogram1D.shape[0]/2 + 5])     #self.histogram1D[:,1]/(np.sum(self.histogram1D[1:-1,1]) + (self.histogram1D[0,1] + self.histogram1D[-1,1])/2)      #self.histogram1D[:,1]/np.sum(self.histogram1D[:,1])
            # scaling it just cuz! (also Valeriani 2007)
#             self.histogram1DNormed[:,1]*=(1.37/4.36)
            histToPlot = self.histogram1DNormed
#             print(self.histogram1DNormed)
            normCheck = np.sum(self.histogram1DNormed*(float(50)/12))
            print('replicate normCheck is: %.10f' % normCheck)
        else:
...
class SweepBiphasics(Biphasics):
    ...
    def FFluxProb(self, axesArr=None, fig=None, figDiagonalLength=6, fontSize=20, fontSizeTicks=14, hideLastTick=False, hideRedundantAxes=True, 
                  hspace=.1, logScaledY=False, normed=False, s=100, saveFig=True, shape=None, wspace=.1):
        self.linesForLegend = []
        if shape==None:
            shape = [len(self.simsSweep)]
        if axesArr==None or fig==None:
        ...
        for i,sims in enumerate(self.simsSweep[::4]):
            sims.FFluxProb(axes=axesArr.ravel()[i], fig=fig, logScaledY=logScaledY, s=s)
...
#             sims.Hist2D(axes=axesArr.ravel()[i], fig=fig, fontSize=fontSize, saveFig=False)
    ...
    def Hist(self, axesArr=None, fig=None, figDiagonalLength=6, fontSize=20, fontSizeTicks=14, hideLastTick=False, hideRedundantAxes=True, 
             hspace=.1, logScaledY=False, normed=False, saveFig=True, shape=None, wspace=.1):
        ...
        for i,sims in enumerate(self.simsSweep):
            line = sims.Hist(axes=axesArr.ravel()[i], fig=fig, fontSize=fontSize, fontSizeTicks=fontSizeTicks, 
                             hideLastTick=hideLastTick, logScaledY=logScaledY, normed=normed, saveFig=saveFig)
...
#             sims.Hist2D(axes=axesArr.ravel()[i], fig=fig, fontSize=fontSize, saveFig=False)
            ...
            fig.delaxes(axesArr.ravel()[i])
            i+=1
        # hack to delete the pendulous extra 11th plot
