...
class FFluxHistsFieldsTestBase(object):
    ffhKey = (('InterfaceTilingID', 19), ('BinTilingIDs', (1,2)))
    ...
    def test_basin_weights(self):
        ...
        bWArr = np.array(self.ffluxHists[self.ffhKey].basin_weights)
        try:
    ...
    def test_order_parameter_values(self):
        ...
        opVArr = np.array(self.ffluxHists[self.ffhKey].order_parameter_values)
    ...
    def test_phase_n_order_parameter_values(self):
        '''
        test the phase_n_order_parameter_values in the histogram that results from a FFluxTrajectoryToOParamHistT transform
        '''
        self.loadData(full=True)
            
        opVArr = np.array(self.ffluxHists[self.ffhKey].phase_n_order_parameter_values['BACKWARD'].order_parameter_values)
        highAArr = np.array([[7.220809863051713e-05, 6.587461566266914e-06, 4.832841849959998e-07, 2.6400704159309915e-07, 2.416420924979999e-07],
                              [0.0001803673385375774, 2.0984137137967825e-06, 3.894788790064249e-06, 1.1741598274927106e-08, 1.2138016997637741e-06],
                              [0.00038718100349412345, 1.37911915413442e-05, 7.430754336846725e-08, 0.0, 1.977754813428722e-06],
                              [0.0003521400664428631, 2.985418777382966e-06, 9.783099682669265e-07, 3.0416330769334984e-08, 9.665683699919996e-07],
    ...
    def test_phase_zero_order_parameter_values(self):
        '''
        test the order_parameter_values in the histogram that results from a FFluxTrajectoryToOParamHistT transform
        '''
        self.loadData(full=True)
            
        opVArr = np.array(self.ffluxHists[self.ffhKey].phase_zero_order_parameter_values['FORWARD'].order_parameter_values)
        highAArr = np.array([[7.220809863051713e-05, 6.587461566266914e-06, 4.832841849959998e-07, 2.6400704159309915e-07, 2.416420924979999e-07],
                              [0.0001803673385375774, 2.0984137137967825e-06, 3.894788790064249e-06, 1.1741598274927106e-08, 1.2138016997637741e-06],
                              [0.00038718100349412345, 1.37911915413442e-05, 7.430754336846725e-08, 0.0, 1.977754813428722e-06],
                              [0.0003521400664428631, 2.985418777382966e-06, 9.783099682669265e-07, 3.0416330769334984e-08, 9.665683699919996e-07],
    ...
    def test_phase_weights(self):
        ...
        pWArr = np.array(self.ffluxHists[self.ffhKey].phase_weights)
    ...
    def test_trajectory_phase_map(self):
        ...
        tPMArr = np.array(self.ffluxHists[self.ffhKey].trajectory_phase_map)
