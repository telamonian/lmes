...
from lm_anal.test.datum.hist import OParamHistsLazyTestBase, OParamHistsFieldsTestBase

import unittest

class SimsTestCase(unittest.TestCase, OParamHistsLazyTestBase, OParamHistsFieldsTestBase):
    '''
    test how the Sims class handles the functions related to loading brute force trajectories, transforming them into 
    '''
    def setUp(self):
        self.cleanUpInt()
        self.sims = Sims(rootPath=testDataPath)
        self.opHists = self.sims['biphasic_switch'].oparamHists
