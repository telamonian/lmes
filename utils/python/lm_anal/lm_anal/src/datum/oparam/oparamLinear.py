...
class OParamLinear(OParam):
    ...
    def init(self):
        self.multiplier = np.zeros((np.max(self.species_ids) - np.min(self.species_ids) + 1))
        for i,coeff in zip((sid - np.min(self.species_ids) for sid in self.species_ids), self.species_coefficients):
...
#             multipliers.append(op.multiplier)
        ...
        newOP.species_coefficients = species_coefficients
        newOP.species_ids = species_ids
