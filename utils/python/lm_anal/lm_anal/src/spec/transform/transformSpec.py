from lm_anal.src.helper import CamelCaseUpper
from lm_anal.src.spec.spec import Spec

__all__ = ['TransformSpec']

class TransformSpec(Spec):
    keywords = {'name', 'srcTypes', 'dstTypes', 'srcDataTypes', 'dstDataTypes', 'srcKeyType', 'dstKeyType', 'propertyTransformSpecs', 'requiredArgs', 'requiredData'}
    
    #requiredKeywords = {'srcTypes', 'dstTypes'}
    # if we got a single srcTypes/dstTypes rather than a list (or whatever), put it in a set. Otherwise, convert to set
    setKeywords = {'srcTypes', 'dstTypes', 'srcDataTypes', 'dstDataTypes', 'srcKeyType', 'dstKeyType', 'requiredArgs', 'requiredData'}
    
    defaultKeywordDict = {'srcTypes':set(),
                          'dstTypes':set(),
                          'srcDataTypes':set(), 
                          'dstDataTypes':set(), 
                          'srcKeyType':set(), 
                          'dstKeyType':set()}
    
    @staticmethod
    def getNamesFromKwargs(kind='src', **kwargs):
        klassSets = [kwargs.get('%sTypes' % kind, set()),
                     kwargs.get('%sDataTypes' % kind, set()),
                     kwargs.get('%sKeyTypes' % kind, set())]
        return [klass.__name__ for klass in set.union(*klassSets)]
            
    def defaultName(self, **kwargs):
        # if we don't specify a name, come up with one from srcTypes + dstTypes -> <ST1>And<ST2>...To<DT1>And<DT2>...
        return 'To'.join(('And'.join(self.getNamesFromKwargs(kind='src', **kwargs)), 
                         'And'.join(self.getNamesFromKwargs(kind='dst', **kwargs))))
#         return 'To'.join(('And'.join(map(lambda x: CamelCaseUpper(x.__name__), kwargs['srcTypes'])), 
#                          'And'.join(map(lambda x: CamelCaseUpper(x.__name__), kwargs['dstTypes']))))