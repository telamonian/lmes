...
from lm_anal.src.datum.probability import TransitionProbability
...
__all__ = ['FFluxOutputToTransitionProbabilityT']

class FFluxOutputToTransitionProbabilityT(BaseT):
    srcTypes = frozenset({FFluxOutput})
    dstTypes = frozenset({TransitionProbability})
    
    transformSpecs = TransformSpecs(TransformSpec(srcTypes={FFluxOutput}, dstTypes={TransitionProbability}, requiredData={'tilings'},
        propertyTransformSpecs=PropertyTransformSpecs(
            PropertyTransformSpec(dstProps='transition_probability', srcProps='probability_i_to_i_plus_one', type='special'))))
