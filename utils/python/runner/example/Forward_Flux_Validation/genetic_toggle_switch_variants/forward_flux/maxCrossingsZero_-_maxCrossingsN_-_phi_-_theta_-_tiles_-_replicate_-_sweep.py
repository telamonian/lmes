...
def GetFFluxInputTups():
    ...
    return [iSCs, iSCBs] + ops + tilings + tilingsSweep
...
if __name__=='__main__':
    ...
    # sampling rate scaling factor (phi) sweep
    sweepTups.append(genSweepTup(ticks=phiTicks, label='phi_%.1e',
                                 paramFunc=lambda phi: []))
    ...
    # number of tiles sweep
    def genTiling(tiles):
        id = 100 + tiles
        numEdges = tiles + 1
        return [Tiling(id=id, orderParameterID=0, type=0, edges=np.linspace(-27, 27, numEdges), isCurrentTiling=True)]
    sweepTups.append(genSweepTup(ticks=tileTicks, label='tiles_%d', paramFunc=genTiling))
